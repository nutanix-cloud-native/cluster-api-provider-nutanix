	...
	_ = Describe("NutanixFailureDomainReconciler", func() {
		...
		BeforeEach(func() {
			...
			fdObj = &infrav1.NutanixFailureDomain{
				...
				Spec: infrav1.NutanixFailureDomainSpec{
					PrismElement: infrav1.PrismElementFailureDomainSpec{
		...
		Context("Update an NutanixFailureDomain Configuration", func() {
			It("should not allow to update failure domain prism element configuration", func() {
				...
				fd2 := fdObj.DeepCopy()
				fd2.Spec.PrismElement.PrismElement.Name = &rstr2
				err := k8sClient.Update(ctx, fd2)
				// Expect error ocurred and the error message contains the expected substr.
				g.Expect(err).To(HaveOccurred())
				g.Expect(err.Error()).To(ContainSubstring("spec.prismElement.prismElement: Invalid value: \"object\": prismElement is immutable once set"))
			})
			...
			It("should not allow to update failure domain subnets configuration", func() {
				...
				fd2 := fdObj.DeepCopy()
				fd2.Spec.PrismElement.Subnets[0].Name = &rstr2
				err := k8sClient.Update(ctx, fd2)
				// Expect error ocurred and the error message contains the expected substr.
				g.Expect(err).To(HaveOccurred())
				g.Expect(err.Error()).To(ContainSubstring("spec.prismElement.subnets: Invalid value: \"array\": subnets is immutable once set"))
			})
		})
