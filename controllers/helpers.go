...
func GetPEUUID(ctx context.Context, client *v4Converged.Client, peName, peUUID *string) (string, error) {
	...
	if peUUID != nil && *peUUID != "" {
		...
		for _, s := range responsePEs {
			if strings.EqualFold(*s.Name, *peName) && hasPEClusterServiceEnabled(&s, serviceNamePECluster) {
...
func HasTaskInProgress(ctx context.Context, client *v4Converged.Client, taskUUID string) (bool, error) {
	...
	if taskStatus.Status.GetName() != taskSucceededMessage {
...
func hasPEClusterServiceEnabled(peCluster *clusterModels.Cluster, serviceName string) bool {
	if peCluster == nil ||
		peCluster.Config == nil ||
		peCluster.Config.ClusterFunction == nil {
		return false
	}
	serviceList := peCluster.Config.ClusterFunction
	for _, s := range serviceList {
		if strings.ToUpper(string(s.GetName())) == serviceName {
			return true
		}
	}
	return false
