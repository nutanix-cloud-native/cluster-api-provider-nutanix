...
func CreateDataDiskList(ctx context.Context, client *prismclientv3.Client, v4client *v4Converged.Client, dataDiskSpecs []infrav1.NutanixMachineVMDisk, peUUID string) ([]*prismclientv3.VMDisk, error) {
	...
	for _, dataDiskSpec := range dataDiskSpecs {
		...
		if dataDiskSpec.StorageConfig != nil {
			...
			if dataDiskSpec.StorageConfig.StorageContainer != nil {
				...
				sc, err := GetStorageContainerInCluster(ctx, v4client, *dataDiskSpec.StorageConfig.StorageContainer, peID)
				if err != nil {
					return nil, err
				}
...
func GetStorageContainerInCluster(ctx context.Context, client *v4Converged.Client, storageContainerIdentifier, clusterIdentifier infrav1.NutanixResourceIdentifier) (*clustermgmtconfig.StorageContainer, error) {
	...
	case storageContainerIdentifier.IsUUID():
		filter = fmt.Sprintf("extId eq '%s'", *storageContainerIdentifier.UUID)
	case storageContainerIdentifier.IsName():
		filter = fmt.Sprintf("name eq '%s'", *storageContainerIdentifier.Name)
	default:
