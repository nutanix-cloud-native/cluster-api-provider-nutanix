...
// +kubebuilder:rbac:groups=infrastructure.cluster.x-k8s.io,resources=nutanixmetrositees,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=infrastructure.cluster.x-k8s.io,resources=nutanixmetrositees/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=infrastructure.cluster.x-k8s.io,resources=nutanixmetrositees/finalizers,verbs=get;update;patch
...
func (r *NutanixMetroSiteReconciler) reconcileDelete(ctx context.Context, metroSite *infrav1.NutanixMetroSite, machines []capiv1beta2.Machine) error {
	...
	if len(usedItems) == 0 {
		v1beta1conditions.MarkTrue(metroSite, infrav1.MetroSiteSafeForDeletionCondition)
	...
	v1beta1conditions.MarkFalse(metroSite, infrav1.MetroSiteSafeForDeletionCondition,
		infrav1.MetroSiteInUseReason, capiv1beta1.ConditionSeverityError, "%s", errMsg)
...
func (r *NutanixMetroSiteReconciler) reconcileNormal(ctx context.Context, metroSite *infrav1.NutanixMetroSite) (reterr error) {
	...
	defer func() {
		if reterr != nil {
			v1beta1conditions.MarkFalse(metroSite, infrav1.MetroSiteValidatedCondition, infrav1.MetroSiteMisconfiguredReason,
				capiv1beta1.ConditionSeverityError, "%s", reterr.Error())
		} else {
			v1beta1conditions.MarkTrue(metroSite, infrav1.MetroSiteValidatedCondition)
		}
	}()
	...
	if _, err := getNutanixFailureDomainObject(ctx, r.Client, metroSite.Spec.FailureDomainRef.Name, metroSite.Namespace); err != nil {
		reterr = err
		return err
	}

	for _, fdRef := range metroObj.Spec.FailureDomains {
		if fdRef.Name == metroSite.Spec.FailureDomainRef.Name {
			reterr = nil
			return nil
		}
	}

	reterr = fmt.Errorf("metroSite spec.failureDomain %s is not in the referred NutanixMetro's failureDomains: %+v", metroSite.Spec.FailureDomainRef.Name, metroObj.Spec.FailureDomains)
	return reterr
}
