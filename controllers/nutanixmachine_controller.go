...
func (r *NutanixMachineReconciler) reconcileDelete(rctx *nctx.MachineContext) (reconcile.Result, error) {
	...
	taskInProgress, err := VmHasTaskInProgress(ctx, convergedClient, targetUUID, nil, rctx.PCVersion)
	if err != nil {
...
func (r *NutanixMachineReconciler) vmToDelete(rctx *nctx.MachineContext, recordedUUID, vmName string) (*vmmconfig.Vm, bool, error) {
	...
	if v3Client == nil || !useMetroDRDeletePath(rctx) {
		vm, err := FindVMByUUID(ctx, rctx.ConvergedClient, recordedUUID, nil, rctx.PCVersion)
		return vm, false, err
	}
...
func (r *NutanixMachineReconciler) resolveRecoveredVMForDelete(rctx *nctx.MachineContext, v3Client prismclientv3.Service, vmName string, waitIfMissing bool) (*vmmconfig.Vm, bool, error) {
	...
	if recoveredUUID := nutanixMachineAnnotation(rctx.NutanixMachine, recoveredVMUUIDAnnotation); recoveredUUID != "" {
		...
		if recoveredDecoupled {
			...
			vm, err := FindVMByUUID(ctx, rctx.ConvergedClient, recoveredUUID, nil, rctx.PCVersion)
			if err != nil {
				return nil, false, err
			}
			if vm == nil {
	...
	live, err := findNonDecoupledVMByName(ctx, rctx.ConvergedClient, v3Client, vmNamesForDelete(vmName, rctx.NutanixMachine.Name), nil, rctx.PCVersion)
	if err != nil {
		return nil, false, err
	}
...
func (r *NutanixMachineReconciler) getOrCreateVM(rctx *nctx.MachineContext, effectiveProject *nctx.ProjectInfo, resourceGroup *multidomainModels.ResourceGroup) (*vmmconfig.Vm, error) {
	...
	vmFound, err := FindVM(ctx, convergedClient, rctx.Machine, rctx.NutanixMachine, vmName, effectiveProjectExtID, rctx.PCVersion, nutanixV3Service(rctx))
	if err != nil {
	...
	vm, err = createAndWaitForVM(v4Converged.WithRequestID(ctx, requestID), rctx, vm, vmName)
	if err != nil {
		return nil, err
	}
