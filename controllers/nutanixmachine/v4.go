...
import (
	...
	// Additional imports needed for V4 implementation:
	// "encoding/base64"
...
func (r *NutanixMachineReconciler) getOrCreateVmV4(nctx *controllers.NutanixExtendedContext, scope *NutanixMachineScope) (*vmmModels.Vm, error) {
	...
	categories, err := r.getOrCreateCategoriesV4(nctx, r.getMachineCategoryIdentifiersV4(scope))
	if err != nil {
	...
	_ = categories // Will be used when vmSpec.Categories is set

	// Step 10: Set Project (if specified)
	err = r.addVMToProjectV4(nctx, scope, vmSpec)
	if err != nil {
	...
	// Step 12: Get disk list (system, bootstrap, data disks)
	...
	// Step 13: Configure VM resources (CPU, Memory, NICs, Disks, GPUs)
	...
	// Step 14: Add guest customization (cloud-init)
	...
	//     r.setFailureStatusV4(scope, createErrorFailureReason, errorMsg)
	//     return nil, err
	// }

	// Step 15: Set boot type (UEFI/Legacy)
	...
	// vmResponse, err := v4Client.CreateVm(ctx, vmSpec)
	// if err != nil {
	...
	// Step 17: Wait for task completion (it will fetch the VM after creation)
	// taskExtId := vmResponse.Data.ExtId  // In V4, task tracking might be different
	// if taskExtId != nil && *taskExtId != "" {
	//     log.Info(fmt.Sprintf("Waiting for task to complete for VM %s", scope.NutanixMachine.Name))
	//     if err := r.waitForTaskCompletionV4(ctx, v4Client, *taskExtId); err != nil {
	//         errorMsg := fmt.Errorf("error occurred while waiting for task to complete: %v", err)
	//         r.setFailureStatusV4(scope, createErrorFailureReason, errorMsg)
	//         return nil, errorMsg
	//     }
	// }
	...
	// vmSpec.PowerState = vmmModels.POWERSTATEON.Ptr()

	// Step 19: Set provider ID and VM UUID
	// vmUuid := *vmResponse.Data.ExtId
	// scope.NutanixMachine.Spec.ProviderID = controllers.GenerateProviderID(vmUuid)
	// scope.NutanixMachine.Status.VmUUID = vmUuid

	// Step 20: Log creation details
	// log.V(1).Info(fmt.Sprintf("Sent the post request to create VM %s. Got the vm UUID: %s", vmName, vmUuid))

	// Step 21: Return the created VM
	// return *vm, nil

	// TODO: Implement all the above steps for V4 API
	//
	// Helper functions that need to be implemented for V4:
	// - r.FindVmV4(nctx, scope, vmName) - Find existing VM using V4 API
	// - r.validateMachineConfigV4(nctx, scope) - Validate config (similar to V3)
	// - r.GetSubnetAndPEUUIDsV4(nctx, scope) - Get subnet and PE UUIDs using V4 API
	// - r.getCategorySpecV4(ctx, v4Client, categoryIdentifiers) - Handle categories in V4
	// - r.getMachineCategoryIdentifiersV4(nctx, scope) - Get category identifiers
	// - r.addVMToProjectV4(nctx, scope, vmSpec) - Add VM to project using V4 API
	// - r.getGPUListV4(ctx, v4Client, gpus, peUUID) - Get GPU list for V4
	// - r.getDiskListV4(nctx, scope, peUUID) - Get all disks (system, bootstrap, data)
	//   - r.getSystemDiskV4(nctx, scope) - Get system disk for V4
	//   - r.getBootstrapDiskV4(nctx, scope) - Get bootstrap disk for V4
	//   - r.getDataDisksV4(nctx, scope, peUUID) - Get data disks for V4
	// - r.addGuestCustomizationToVMV4(nctx, scope, vmSpec) - Add cloud-init data
	// - r.addBootTypeToVMV4(nctx, scope, vmSpec) - Set UEFI/Legacy boot
	// - r.setFailureStatusV4(scope, reason, err) - Set failure status
	// - r.waitForTaskCompletionV4(ctx, v4Client, taskId) - Wait for task completion
	// - r.findVMByUUIDV4(ctx, v4Client, vmUuid) - Find VM by UUID after creation
	//
	// Notes for V4 implementation:
	// - Use vmmModels.* types instead of prismclientv3.* types
	// - Field names are different (ExtId vs UUID, etc.)
	// - API methods are different (CreateVm vs CreateVM)
	// - Task handling in v4 facade client use TaskWaiter[vmmModels.Vm] which wait for the VM to be created and then fetch the VM and return it
	// - Category handling might use different V4 endpoints
	// - Project assignment might use different V4 mechanisms
	// - GPU and Disk configurations use different V4 structures

	err = fmt.Errorf("getOrCreateVmV4 not implemented yet")
	return vm, err
}
...
func (r *NutanixMachineReconciler) FindVmV4(nctx *controllers.NutanixExtendedContext, scope *NutanixMachineScope, vmName string) (*vmmModels.Vm, error) {
	...
	if vmUUID == "" {
		...
		if len(vm) == 0 {
			return nil, fmt.Errorf("no VM found with name %s", vmName)
		}
...
func (r *NutanixMachineReconciler) getOrCreateCategoriesV4(nctx *controllers.NutanixExtendedContext, categoryIdentifiers []*infrav1.NutanixCategoryIdentifier) ([]v4prismModels.Category, error) {
	ctx := nctx.Context
	v4Client := nctx.GetV4FacadeClient()

	categories := make([]v4prismModels.Category, 0)
...
func (r *NutanixMachineReconciler) addVMToProjectV4(nctx *controllers.NutanixExtendedContext, scope *NutanixMachineScope, vmSpec *vmmModels.Vm) error {
	...
	vmSpec.Project = vmmModels.NewProjectReference()
	vmSpec.Project.ExtId = &projectUUID //TODO: Check if it works in all PC version after v4 beta

	log.V(1).Info("Project lookup successful - V4 VM creation will proceed without direct project assignment",
		"vmName", vmName,
		"projectUUID", projectUUID,
		"note", "V4 VMs may be assigned to projects via different mechanism after creation")
