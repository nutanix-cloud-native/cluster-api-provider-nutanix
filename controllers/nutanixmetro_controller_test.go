...
var _ = Describe("NutanixMetroReconciler", func() {
	...
	BeforeEach(func() {
		...
		metroSiteObj = &infrav1.NutanixMetroSite{
			...
			Spec: infrav1.NutanixMetroSiteSpec{
				PreferredFailureDomain: corev1.LocalObjectReference{Name: fdObj1.Name},
	...
	Context("Test reconcileDelete", func() {
		...
		It("should allow deletion when not referenced by other objects", func() {
			...
			Expect(cond.Reason).To(Equal(infrav1.MetroNotInUseReason))
	...
	Context("Test pause", func() {
		It("should be a no-op when the NutanixMetro carries the paused annotation", func() {
			metroObj.Annotations = map[string]string{capiv1beta2.PausedAnnotation: "true"}
			Expect(k8sClient.Create(ctx, metroObj)).To(Succeed())

			res, err := reconciler.Reconcile(ctx, reconcile.Request{
				NamespacedName: client.ObjectKeyFromObject(metroObj),
			})
			Expect(err).NotTo(HaveOccurred())
			Expect(res).To(Equal(reconcile.Result{}))

			// The finalizer must not be added while paused.
			current := &infrav1.NutanixMetro{}
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(metroObj), current)).To(Succeed())
			Expect(ctrlutil.ContainsFinalizer(current, infrav1.NutanixMetroFinalizer)).To(BeFalse())
		})
	})
})
