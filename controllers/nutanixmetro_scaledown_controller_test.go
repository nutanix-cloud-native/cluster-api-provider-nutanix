...
func TestMetroScaleDownBalancerMapClusterToMachineSets(t *testing.T) {
	g := NewWithT(t)
	r := newBalancerReconciler(g,
		balancerMS(metroFailureDomainPrefix+"metro0", nil),
		&capiv1beta2.MachineSet{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "ms-other-cluster",
				Namespace: placementNamespace,
				Labels:    map[string]string{capiv1beta2.ClusterNameLabel: "other-cluster"},
			},
		},
		&capiv1beta2.MachineSet{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "ms-other-namespace",
	...
	reqs := r.mapClusterToMachineSets()(context.Background(), balancerCluster())
	g.Expect(reqs).To(HaveLen(1))
	g.Expect(reqs[0].NamespacedName).To(Equal(client.ObjectKey{
		Name:      balancerMSName,
		Namespace: placementNamespace,
	}))
}
