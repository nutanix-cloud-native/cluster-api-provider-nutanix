...
release-manifests: kustomize manifests cluster-templates
	mkdir -p $(RELEASE_DIR)
...
cluster-e2e-templates-v152: kustomize ## Generate cluster templates for CAPX v1.5.2
...
prepare-local-clusterctl: kustomize manifests cluster-templates  ## Prepare overide file for local clusterctl.
...
PHONY: tools-bin-dir
tools-bin-dir: $(TOOLS_BIN_DIR) ## Location to install tooling binaries.
$(TOOLS_BIN_DIR):
	mkdir -p $(TOOLS_BIN_DIR)

.PHONY: controller-gen
controller-gen: $(CONTROLLER_GEN) ## Install controller-gen if missing.
$(CONTROLLER_GEN): tools-bin-dir
	GOBIN=$(TOOLS_BIN_DIR) go install sigs.k8s.io/controller-tools/cmd/controller-gen@$(CONTROLLER_GEN_VER)

.PHONY: conversion-gen
conversion-gen: $(CONVERSION_GEN) ## Install conversion-gen if missing.
$(CONVERSION_GEN): tools-bin-dir
	GOBIN=$(TOOLS_BIN_DIR) go install k8s.io/code-generator/cmd/conversion-gen@$(CONVERSION_GEN_VER)

.PHONY: kustomize
kustomize: $(KUSTOMIZE) ## Install kustomize if missing.
$(KUSTOMIZE): tools-bin-dir
