...
const (
	...
	NutanixFailureDomainFinalizer = "nutanixfailuredomain.infrastructure.cluster.x-k8s.io"
)

// PrismElementFailureDomainSpec defines Prism Element specific failure domain spec.
type PrismElementFailureDomainSpec struct {
	// prismElement is to identify the Prism Element in the Prism Central for the failure domain.
	// +kubebuilder:validation:XValidation:rule="self == oldSelf",message="prismElement is immutable once set"
...
type NutanixFailureDomainSpec struct {
	// prismElement defines Prism Element specific failure domain configuration.
	// +kubebuilder:validation:Required
	PrismElement PrismElementFailureDomainSpec `json:"prismElement"`
}
